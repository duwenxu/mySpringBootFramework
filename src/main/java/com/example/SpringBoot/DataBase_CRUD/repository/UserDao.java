...
public interface UserDao extends JpaRepository<User, Integer> {
     ...
     * 　　　　3、注意JPQL不支持INSERT操作。
     * @Modifying需要和@Transactional配合使用才能正常使用
     * 原因：
     ...
     *
     * @Transactional注解
     * 源码中：SimpleJpaRepository在类级别定义了@Transactional(readOnly = true)，而在和save、delete相关的操作重写了@Transactional属性（重写默认的readOnly属性是false），其余查询操作readOnly仍然为true
     */
